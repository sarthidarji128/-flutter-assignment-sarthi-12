...
router.post('/',async(request,response)=>{
    try{
              ...
              response.status(201).json({message:"Flower added successsfully"});
    }
    catch(error){
             response.status(500).json({message:error.message});
    }
...
router.put('/:id',async(request,respons)=>{
    try{
         const flower =await Flower.findByIdAndUpdate(request.params.id,request.body,{new:true});
           if(!flower){
            return response.status(404).json({message:"Flower not found"});
        }
